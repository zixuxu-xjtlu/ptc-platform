  ...
  <div class="user-manage-container">
    <div class="page-header mb-24 animate-down">
      <div class="title-section">
        <h2>👥 成员档案与积分中心</h2>
        <p class="subtitle">管理全员身份权限，当前已实现【身份权重排序】与【批量快捷奖惩】</p>
      </div>
      ...
      <el-table :data="sortedFilteredUsers" style="width: 100%" v-loading="loading" stripe class="modern-table">
        <el-table-column label="权限等级" width="110">
          <template #default="scope">
            ...
            <el-tag v-else type="info" variant="plain" round class="role-tag">普通</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="成员信息" min-width="150">
          <template #default="scope">
            <div class="user-info-cell">
              <el-avatar :size="32" class="user-avatar hide-on-mobile" :style="getAvatarStyle(scope.row.role)">
        ...
        <el-table-column label="账目管理" width="120" align="right" fixed="right">
          <template #default="scope">
            <div class="btn-group">
              <el-button size="small" type="primary" link @click="openPointsDialog(scope.row, 'normal')">改分</el-button>
              <el-button v-if="scope.row.role !== 'user'" size="small" type="success" link @click="openPointsDialog(scope.row, 'work')">发奖</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
...
const sortedFilteredUsers = computed(() => {
  const roleWeights = { admin: 100, member: 50, user: 10 }
  let result = users.value.filter(u => 
    (u.realName || '').includes(searchKey.value) || 
    (u.username || '').includes(searchKey.value)
  )
  return result.sort((a, b) => (roleWeights[b.role] || 0) - (roleWeights[a.role] || 0))
})
...
const submitBatchPoints = async () => {
  ...
  if (nameList.length === 0) return ElMessage.warning('名单格式不正确')
  ...
  try {
    ...
    if (targets.length === 0) {
      ...
      return ElMessage.error('找不到对应的实名用户，请检查')
    }
    ...
    ElMessage.success(`批量更新成功！操作 ${targets.length} 人`)
...
.modern-radio :deep(.el-radio-button__inner) { border-radius: 8px; margin: 0 5px; font-size: 12px;}
...
@media screen and (max-width: 768px) {
  ...
  .search-input:focus-within { width: 100%; }
  .btn-group { flex-direction: row; justify-content: flex-end; }
}
</style>