  ...
  <div class="quiz-manage-container">
    <div class="page-header mb-24 fade-in-down">
      <div class="header-left">
        <h2>🧠 智学题库与套卷管理</h2>
        <p class="subtitle">管理学科套卷、配置分值与解析，实时追踪全站正确率</p>
      </div>
      ...
      <el-button v-if="activeTab === 'sets'" type="primary" size="large" round class="publish-btn shadow-glow" @click="handleOpenSetForm()">
        <el-icon class="mr-1"><Plus /></el-icon> 新建套卷
      </el-button>
    </div>
      ...
      <el-tab-pane name="sets">
        ...
        <el-card shadow="never" class="main-card">
          <el-table :data="quizSets" v-loading="loading" stripe class="modern-table" :row-key="row => row._id">
            ...
            <el-table-column prop="title" label="套卷标题" min-width="180" />
            <el-table-column label="及格线 / 奖励" width="150" align="center">
              <template #default="scope">
                <div class="text-sm">
                  <span class="font-bold text-gray-700">{{ scope.row.passScore || 60 }}分</span> 及格
                  <br>
                  <span class="text-warning font-bold">+{{ scope.row.rewardPoints || 20 }} 积分</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="发布状态" width="120" align="center">
              <template #default="scope">
                <el-switch 
                  ...
                  @change="updateSetStatus(scope.row)"
                />
              </template>
            </el-table-column>
      ...
      <el-tab-pane name="questions">
        ...
        <el-card shadow="never" class="main-card">
          <el-table :data="questionsWithStats" v-loading="loading" stripe class="modern-table" :row-key="row => row._id">
            ...
            <el-table-column label="所属套卷" width="160" show-overflow-tooltip>
              <template #default="scope">
                <el-tag size="small" type="info" effect="plain">{{ getSetName(scope.row.quizSetId) }}</el-tag>
              </template>
            </el-table-column>

            <el-table-column label="题型/分值" width="100">
              <template #default="scope">
                <div>
                  <el-tag size="small" :type="scope.row.type === 'choice' ? 'primary' : 'warning'" effect="plain" class="mb-1">
                    {{ scope.row.type === 'choice' ? '选择题' : '填空题' }}
                  </el-tag>
                  <div class="text-xs text-gray-500">{{ scope.row.score || 10 }} 分</div>
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="正确率" width="140">
              <template #default="scope">
                <div class="flex items-center gap-2">
                  <el-progress :percentage="scope.row.stats.accuracy" :color="customColors" :stroke-width="8" :show-text="false" class="flex-1" />
    ...
    <el-dialog v-model="setFormVisible" :title="isEditingSet ? '编辑套卷' : '新增学科套卷'" width="500px">
      <el-form label-position="top">
        <el-form-item label="所属学科名称" required>
          <el-input v-model="setFormData.subject" placeholder="例如：高等数学" />
        </el-form-item>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="及格分数线">
              <el-input-number v-model="setFormData.passScore" :min="1" :max="1000" class="w-full" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="及格奖励积分">
              <el-input-number v-model="setFormData.rewardPoints" :min="0" :max="500" class="w-full" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-dialog v-model="formVisible" :title="isEditing ? '编辑题目' : '新增题目'" width="900px" custom-class="latex-dialog" :close-on-click-modal="false" top="5vh">
        ...
        <div class="editor-pane pr-4 border-r border-gray-100">
          <el-form label-position="top">
            <el-row :gutter="20">
              ...
              <el-col :span="8">
                <el-form-item label="本题分值" required>
        ...
        <div class="preview-pane pl-4 bg-gray-50 rounded-lg p-4 h-full overflow-y-auto">
          ...
          <div class="preview-card bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
            <div class="mb-4">
              ...
              <span class="text-gray-400 text-xs ml-1">({{ formData.score || 10 }} 分)</span>
              <div class="math-preview mt-2" v-html="formData.content || '暂无题干'"></div>
            </div>
...
const customColors = [{ color: '#f56c6c', percentage: 40 }, { color: '#e6a23c', percentage: 70 }, { color: '#5cb87a', percentage: 100 }]
    ...
    questionsWithStats.value = rawQuestions.value.map(q => {
      ...
      allRecords.forEach(record => {
        ...
        if (userAns !== undefined && userAns !== '') {
          totalAnswers++
          if (userAns.toString().trim().toLowerCase() === q.correctAnswer.toString().trim().toLowerCase()) correctAnswers++
        }
      })
...
const getSetName = (setId) => {
  ...
  return set ? `[${set.subject}] ${set.title}` : '未知套卷'
}
const formatDate = (ts) => {
  if (!ts) return ''
  const d = new Date(ts)
...
const setFormData = ref({ _id: '', subject: '', title: '', passScore: 60, rewardPoints: 20, status: 'draft' })
...
const handleOpenSetForm = (set = null) => {
  if (set) {
    ...
    setFormData.value = { _id: '', subject: '', title: '', passScore: 60, rewardPoints: 20, status: 'draft' }
  }
...
const saveSet = async () => {
  ...
  try {
    const payload = {
      ...
      passScore: setFormData.value.passScore,
      rewardPoints: setFormData.value.rewardPoints,
      status: setFormData.value.status || 'draft',
    ...
    setTimeout(() => fetchAllData(), 800) // 延迟确保云端索引刷新
  } catch (error) { ElMessage.error('保存失败') } finally { submitting.value = false }
}

// 开关切换发布状态
const updateSetStatus = async (row) => {
  try {
    await db.collection('quiz_sets').doc(row._id).update({ status: row.status })
    ElMessage.success(row.status === 'published' ? '试卷已对外发布！' : '试卷已撤回，用户不可见。')
  } catch (err) {
    row.status = row.status === 'published' ? 'draft' : 'published' // 失败回滚
    ElMessage.error('状态切换失败')
  }
}

const handleDeleteSet = (id) => {
  ElMessageBox.confirm('确定删除该套卷吗？题目仍保留。', '警告', { type: 'warning' })
    .then(async () => {
      try {
...
const formData = ref({
  _id: '', quizSetId: '', type: 'choice', content: '', score: 10,
  options: [{ label: 'A', text: '' }, { label: 'B', text: '' }, { label: 'C', text: '' }, { label: 'D', text: '' }],
  correctAnswer: '', explanation: ''
})

const handleOpenQuestionForm = (q = null) => {
  if (quizSets.value.length === 0) return ElMessage.warning('请先去“套卷管理”中创建一个套卷！')
  if (q) {
    isEditing.value = true
    formData.value = JSON.parse(JSON.stringify(q))
    if (!formData.value.score) formData.value.score = 10
    if (!formData.value.options) formData.value.options = []
  } else {
    isEditing.value = false
    formData.value = {
      _id: '', quizSetId: quizSets.value[0]._id, type: 'choice', content: '', score: 10,
      options: [{ label: 'A', text: '' }, { label: 'B', text: '' }, { label: 'C', text: '' }, { label: 'D', text: '' }],
      correctAnswer: '', explanation: ''
    }
  }
...
const addOption = () => {
  ...
  formData.value.options.push({ label: labels[formData.value.options.length] || '?', text: '' })
}
...
const saveQuestion = async () => {
  if (!formData.value.quizSetId || !formData.value.content || !formData.value.correctAnswer) return ElMessage.warning('请填写必填项')
  submitting.value = true
  const payload = {
    quizSetId: formData.value.quizSetId,
    type: formData.value.type,
    score: formData.value.score, // 保存分值
...
const handleDeleteQuestion = (id) => {
  ElMessageBox.confirm('确定删除这道题吗？', '确认', { type: 'warning' }).then(async () => {
    await db.collection('questions').doc(id).remove()
    ElMessage.success('已删除'); fetchAllData()
  }).catch(() => {})
}
</script>
